...
export function importAllLocales(): LocaleMessages<VueMessageType> {
    ...
    try {
        ...
        // 导入 @/locales 下文件，不包含子目录，文件名为：([a-z]{2})-?([A-Z]{2})?\.ts
        const i18nRequireContext: __WebpackModuleApi.RequireContext = require.context('../i18n', true, /[/\\]locales[/\\]([a-z]{2})-?([A-Z]{2})?\.ts$/);
        i18nRequireContext.keys().forEach(fileName => {
            // 获取内容
            const modulesConent = i18nRequireContext(fileName);
            if (modulesConent.default) {
                // 获取 PascalCase 命名
                const modulesName = fileName.replace(/(.*\/)*([^.]+).*/ig, "$2");
                if (modules[modulesName]) {
